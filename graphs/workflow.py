...
def route_after_input(state):
    return END if state.get("blocked") else "classifier"


def route_after_output(state):
    return END if state.get("blocked") else "jira_update"


builder = StateGraph(ResolveOpsState)

builder.add_node("input_middleware", input_middleware_node)
...
builder.add_edge("solution", "output_middleware")

builder.add_conditional_edges(
    "output_middleware",
    route_after_output,
    {"jira_update": "jira_update", END: END},
)
